...
async function main() {
  ...
  const fieldUrls = [
    `/settings/fields?module=cm_salesperson_profile&organization_id=${organization_id}`,
    `/settings/customfields?module=cm_salesperson_profile&organization_id=${organization_id}`,
    `/settings/modules/3331482000181494001/fields?organization_id=${organization_id}`,
    `/settings/modules/3331482000181494001/layouts/3331482000181494005?organization_id=${organization_id}`
  ];

  for (const u of fieldUrls) {
    console.log(`\nGET ${u}`);
    const res = await httpsRequest(`https://www.zohoapis.com/books/v3${u}`, { method: 'GET', headers });
