...
async function main() {
  ...
  console.log('--- Checking Custom Modules / Settings ---');
  const modulesUrl = `https://www.zohoapis.com/books/v3/settings/custommodules?organization_id=${organization_id}`;
  const modRes = await httpsRequest(modulesUrl, { method: 'GET', headers });
  console.log('Custom Modules API status:', modRes.statusCode);
