...
async function main() {
  ...
  console.log(`=== Querying /settings/modules (Org: ${organization_id}) ===`);
  const modulesUrl = `https://www.zohoapis.com/books/v3/settings/modules?organization_id=${organization_id}`;
  const modRes = await httpsRequest(modulesUrl, { method: 'GET', headers });
  
  console.log(`Status: ${modRes.statusCode}`);
  console.log(JSON.stringify(modRes.body, null, 2));

  // If there are modules, let's also query individual module details
  const modules = modRes.body?.modules || [];
  for (const mod of modules) {
    console.log(`\n--- Inspecting details for module: ${mod.api_name || mod.module_name || mod.name} ---`);
    const detailUrl = `https://www.zohoapis.com/books/v3/settings/modules/${mod.api_name || mod.module_name || mod.name}?organization_id=${organization_id}`;
    const detailRes = await httpsRequest(detailUrl, { method: 'GET', headers });
