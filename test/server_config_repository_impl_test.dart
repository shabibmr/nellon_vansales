...
void main() {
  group('ServerConfigRepositoryImpl', () {
    ...
    test('returns true immediately if ZohoApiClient already has credentials',
        () async {
      ...
      expect(repository.hasCredentials, isTrue);

      final result = await repository.ensureCredentialsLoaded();
      expect(result, isTrue);
      expect(licenseService.fetchCount, 0);
    });

    test('loads from secure storage cache on warm boot without remote fetch',
        () async {
